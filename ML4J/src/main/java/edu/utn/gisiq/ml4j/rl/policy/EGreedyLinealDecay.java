 ...
 */
public class EGreedyLinealDecay implements Policy{
    private final MersenneTwisterFast mt;
    private final double epsilonMin;
    private final double epsilonDecay;
    private double epsilon;

    public EGreedyLinealDecay(double initialEpsilon, double epsilonMin, double epsilonDecay) {
    ...
    public int chooseAction(INDArray qValues, int episode) {
        ...
        double e = Math.max(epsilonMin, epsilon);
        
        if(mt.nextDouble()<=e){
            //explore
            rta = mt.nextInt(qValues.columns()-1);            
        }else{
            //explote
            rta = idx;
        }
